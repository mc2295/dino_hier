...
if dataset_name == "bm":
    image_path_train = "bm_train.csv"
    image_path_test = "bm_test.csv"

if dataset_name == "rbc":
    image_path_train = "rbc_test.csv"
    image_path_test = "rbc_train.csv"

if dataset_name == "bodyfluid":

    image_path_train = "bodyfluid_train.csv"
    image_path_test = "bodyfluid_test.csv"

if dataset_name == 'hicervix':

    image_path_train = "cerv_HiCervix_train.csv"
    image_path_test = "cerv_HiCervix_test.csv"
model_list = [
    'path/to/model/checkpoints'
]

model_name_list = ['dinov2_vitl14']* 5 # 

for model_path, model_name in zip(model_list, model_name_list):
    ...
    #df = df.sample(10000, random_state=42) # for large datasets
    ...
    for i in range(n_bootstrap):
        ...
        logreg_dir = Path(result_dir) /f"{dataset_name}" / "logreg"
        ...
        knn_dir =Path(result_dir) / f"{dataset_name}" / "knn"
        ...
        for pred, label, metric, results in zip(
            ...
            if dataset_name == 'bm':
                ...
                lca, spearman, hP, hR, hF1, lineage_acc, rare_classes_acc = 0,0,0,0,0,0,0
            results.append({
    ...
    mean_1NN.to_csv(result_dir + f'/{dataset_name}/mean_1NN.csv')
    std_1NN.to_csv(result_dir + f'/{dataset_name}/bs_1NN.csv')
    mean_20NN.to_csv(result_dir + f'/{dataset_name}/mean_20NN.csv')
    std_20NN.to_csv(result_dir + f'/{dataset_name}/bs_20NN.csv')
    mean_logreg.to_csv(result_dir + f'/{dataset_name}/mean_logreg')
    std_logreg.to_csv(result_dir + f'/{dataset_name}/bs_logreg.csv')

    all_result_mean = pd.concat([mean_1NN, mean_20NN, mean_logreg])
    all_result_mean.to_csv(result_dir + f'/{dataset_name}/all_result_mean.csv')
    all_result_std = pd.concat([std_1NN, std_20NN, std_logreg])
    all_result_std.to_csv(result_dir + f'/{dataset_name}/all_result_bs.csv')
