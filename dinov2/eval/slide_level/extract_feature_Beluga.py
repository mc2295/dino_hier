...
def save_features_and_labels_DresdenBMC(task_configs, save_dir, checkpoint, args):
    ...
    # get all images for features extraction
    patient_csv = pd.read_csv(task_configs[args.dataset]['csv'])
    m_subtype_dict = task_configs[args.dataset]['m_subtype']['label_dict']
    npm1_status_dict = task_configs[args.dataset]['npm1_status']['label_dict']
    ext = task_configs[args.dataset]['ext']
    data_dir = task_configs[args.dataset]['root']
    ...
    for patient, m_subtype, npm1_status in tqdm(zip(patient_csv['Patient_ID'],patient_csv['m_subtype'], patient_csv['npm1_status'])):
        patient_path = Path(data_dir) / f'{patient}'
        images = list(Path(patient_path).rglob(f'*{ext}'))
        images = [img for img in images if 'ipynb_checkpoints' not in str(img)]

        label_m_subtype = m_subtype_dict[m_subtype]
        label_npm1_status = npm1_status_dict[npm1_status]
        ...
        if os.path.isfile(h5_filename):  # check if file exists already
            ...
            if not embeddings_list:
                ...
                continue
            embeddings = np.concatenate(embeddings_list,axis=0)
            save_h5_modified(h5_filename, embeddings, label_m_subtype,label_npm1_status)


def save_h5_modified(h5_filename, embeddings, label_m_subtype, label_npm1_status):
    Path(h5_filename).parent.mkdir(parents=True, exist_ok=True)
    with h5py.File(h5_filename, "w") as hf:
        hf.create_dataset("features", data=embeddings)
        hf.create_dataset("m_subtype", data=label_m_subtype)
        hf.create_dataset("npm1_status", data=label_npm1_status)