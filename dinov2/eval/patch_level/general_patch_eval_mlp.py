...
parser.add_argument(
    "--model_name",
    help="name of model",
    default="dinov2_vits14_classifier",
    type=str,
)
...
parser.add_argument(
    ...
    default="/ictstr01/home/icb/valentin.koch/dinov2/debug",
    type=str,
)
...
def save_features_and_labels(feature_extractor, dataloader, save_dir,dataset_len):
    ...
    with torch.no_grad():
        ...
        for images, labels, names in tqdm(dataloader):
            ...
            batch_features, predictions = feature_extractor(images)

            labels_np = labels.numpy()

            for img_name, img_features, img_label,prediction in zip(names, batch_features, labels_np,predictions):
                ...
                with h5py.File(h5_filename, "w") as hf:
                    ...
                    hf.create_dataset("predictions", data=np.argmax(prediction.cpu().numpy()))
...
def main(args):
    ...
    label_dict = OmegaConf.load(args.config_file).label_dict 
    ...
    if model_name in ["owkin","resnet50","resnet50_full","remedis","imagebind"]:
        ...
        sorted_paths = list(Path(args.model_path).rglob("*student_checkpoint.pth"))
    else:
    ...
    for checkpoint in sorted_paths:
        ...
        dataset = PathImageDataset(args.dataset_path, transform=transform, filetype=args.filetype, class_to_label=label_dict)
        ...
        data, predictions, labels=get_data(all_features)

        mlp_dir = parent_dir/ (args.wandb_project+"mlp_eval")
        ...
        wandb.log(mlp_metrics, step=step)
...
def process_file(file_name):
    with h5py.File(file_name, "r") as hf:
        ...
        label = int(hf["labels"][()])
        prediction= int(hf["predictions"][()])
    return features, prediction, label


# {"Accuracy": accuracy, "Balanced_Acc": balanced_acc, "Weighted_F1": weighted_f1}

def evaluate_mlp(predictions,labels):
...
def get_data(all_data):
    ...
    with ThreadPoolExecutor() as executor:
        ...
        for i, future in tqdm(enumerate(futures), desc="Loading data"):
            feature, prediction, label = future.result()
...
def train_and_evaluate_logistic_regression(train_data, train_labels, test_data, test_labels, save_dir, max_iter=1000):
    ...
    logistic_reg = LogisticRegression(C=1 / l2_reg_coef, max_iter=max_iter, solver="lbfgs")
