# used to evaluate the wbc-att dataset. currently doesnt make a lot of sense, knn+logreg wont work as dataset is too small
...
parser.add_argument(
    ...
    default="/ictstr01/groups/shared/users/peng_marr/DinoBloomv2/wbcatt/submission/pbc_attr_v1_train.csv",
    type=str,
)
...
parser.add_argument(
    ...
    default="/ictstr01/groups/shared/users/peng_marr/DinoBloomv2/wbcatt/submission/pbc_attr_v1_test.csv",
    type=str,
)
...
def main(args):
    ...
    for checkpoint in sorted_paths:
        ...
        feature_dir_train = parent_dir / args.experiment_name / "features_train"
        feature_dir_test = parent_dir / args.experiment_name / "features_test"
        
        dataset_train = WbcAttDataset(args.dataset_path,args.train_split, transform=transform)
        dataloader_train = DataLoader(dataset_train, batch_size=args.batch_size, shuffle=False, num_workers=args.num_workers)
        save_features_and_labels(feature_extractor, dataloader_train, feature_dir_train,len(dataset_train))

        dataset_test = WbcAttDataset(args.dataset_path, args.test_split, transform=transform)
        dataloader_test = DataLoader(dataset_test, batch_size=args.batch_size, shuffle=False, num_workers=args.num_workers)
        save_features_and_labels(feature_extractor, dataloader_test, feature_dir_test,len(dataloader_test))
