...
def collate_rep_aug(*args):
    batch_imgs, batch_labels, batch_img_path, batch_labels_cat = default_collate(*args)
...
class CollateMixup(Mixup):
    ...
    def __init__(self, extend_batch=True,unlab_label = -1, **kwargs):
        super().__init__(**kwargs)
        self.unlab_label = unlab_label
        self.extend_batch = extend_batch

    def __call__(self, samples, _=None):

        batch, labels = collate_rep_aug(samples)
        mask = labels != self.unlab_label
        batch = batch[mask]
        labels = labels[mask]
